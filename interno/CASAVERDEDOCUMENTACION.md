...
`config/sitio` → `whatsapp`, `textos {pt|es|en: {clave: texto}}`, `ocultos []`,
`opiniones []`, `airbnb {c1|c2|c3: {url, nota, resenas}}` — **lectura PÚBLICA** (el sitio
no tiene login), escritura de `contenido`. Es el único lugar donde se edita el contenido
público del sitio.
`config/traduccion` → `origen {pt|en: {id: el español con el que se tradujo}}`, `notas`
`config/integraciones` → `googleApiKey`

**Sobre `textos` (T11.45).** No son nueve claves fijas: son **todas las que el sitio
marque con `data-t`**, hoy 108 por idioma. Las nueve del formulario de `cabanas.html`
son un subconjunto histórico; el editor visual edita cualquier `data-t`. Agregar una
clave es agregarla al HTML y a `interno/textos-sitio.js`, y nada más.

**⚠ LAS CLAVES NO PUEDEN LLEVAR PUNTO (T11.46).** El editor guarda con `updateDoc` y el
camino `textos.es.LACLAVE`: Firestore lee **cada punto del camino como un nivel de
anidamiento**. Una clave `casa.r.ruido.t` se guardaría en `textos→es→casa→r→ruido→t`
mientras el sitio lee la clave literal — se guarda bien, dice "✓ Sitio actualizado" y
no se muestra nunca. Se separan con **guión bajo**: `casa_r_ruido_t`. `editar.html`
tiene un guard que se niega a guardar una clave con punto.

**Sobre `ocultos` (T11.53).** Lista de claves que el sitio **no muestra**. Es un campo
aparte de `textos` a propósito: vaciar un texto lo devuelve al diccionario —lo
correcto cuando alguien borra sin querer— mientras que sacar algo del sitio es otra
intención. `textos` guarda lo que dice; `ocultos`, lo que no se muestra. El sitio
esconde el texto **y el contenedor cuyos textos estén todos escondidos**, para que no
quede un renglón vacío con el ícono suelto.

**Sobre `opiniones` y `airbnb` (T11.55).** Las reseñas copiadas a mano de Airbnb y, por
anuncio, su dirección, su nota y cuántas reseñas tiene. Van acá y **no en una colección
nueva**: el sitio ya baja este documento, así que no hay regla de Firestore nueva ni una
segunda lectura por visita.
...
**No van en `textos-sitio.js`**: ese archivo son etiquetas de interfaz y una opinión es un
dato que crece — cada una nueva sería editar código en tres idiomas y subir el service
worker. **Y no se traducen**: una opinión es testimonio ajeno, y pasarla por un traductor
la convierte en texto nuestro y deja de valer como prueba. Se muestra en el idioma en que
la escribieron, con la etiqueta al lado.

**La dirección del anuncio SÍ es contenido público**, a diferencia del `.ics` que se sacó
de `cabanas` en la T11.44. Aquello era higiene —esa colección la baja cada visitante—;
una URL de anuncio *está hecha* para mostrarse.

**Sobre `config/traduccion` (T11.46).** Guarda **el español con el que se hizo cada
traducción**, entero y por idioma destino. Sin eso no hay forma de distinguir "no está
traducido" de "se tradujo, después se corrigió el español y el portugués quedó
diciendo otra cosa" — el caso que se cuela sin que nadie lo note, porque en pantalla
el texto está y parece completo. Se guarda el texto y no una huella para poder mostrar
**qué** cambió. Va en documento aparte y no en `config/sitio` porque ese lo baja cada
visitante del sitio público, sin sesión.
...
## 7bis. DE DÓNDE SALE EL NÚMERO DE TANDA

**No se deduce del briefing del conocimiento del proyecto** — esa copia se queda atrás y
en agosto de 2026 terminaba en la v5.68 / T11.23, con veintisiete tandas de diferencia.
**Tampoco del último commit del repositorio**: puede haber trabajo en curso sin subir.

**Sale del `sw.js` que se está por entregar.** Su `VERSION` es el único contador que
avanza exactamente una vez por tanda y que nadie puede saltear, porque sin tocarlo la
entrega no llega a los teléfonos. Si el `sw.js` va a la v96, la tanda es la que sigue a
la que dejó la v95.

> Se numeró mal dos veces seguidas en agosto de 2026: primero 11.46–11.48 (ya usados) y
> después 11.51–11.53 (usados también, por otra sesión en paralelo). Quedaron como
> 11.55–11.57. Las dos veces el número se dedujo de una fuente que no era el `sw.js`.
>
> **Y hubo un tercer motivo, este documental:** el título del registro v5.68 decía
> *"T11.18 a T11.23"* cuando el registro **cubre hasta la T11.44**. Quien miraba el
> índice concluía que el briefing terminaba veinte tandas antes de donde termina. El
> título se corrigió en la v5.70. **Un título de registro que no dice hasta dónde llega
> hace que el documento mienta desde el índice**, que es donde más se lo consulta: al
> cerrar un registro, el título se ajusta a la última tanda que contiene.

**Cuando hay dos sesiones trabajando a la vez**, el orden real lo dicta la `VERSION` del
service worker, no el número de tanda que cada una se puso.

---

## 8. TRAMPAS CONOCIDAS

- **Una clave con punto en `updateDoc`**: Firestore lee cada punto del camino como un
  nivel de anidamiento. `textos.es.casa.r.ruido` se guarda en cinco niveles mientras el
  código lee la clave literal. **Se guarda bien, no da error, avisa "actualizado" y no
  se muestra nunca.** Claves con guión bajo (T11.46).
- **Un reemplazo masivo por patrón alcanza más de lo que se quería**: renombrar
  `casa.algo` → `casa_algo` también convirtió la cadena `casa.html` dentro de
  `la-casa.html`, porque el guión cuenta como límite de palabra. Quedaron rotos los
  enlaces, el `canonical` y el `og:url`. **Un reemplazo por patrón se verifica contra
  las rutas y las URLs del archivo, no solo contra lo que se quería tocar** (T11.47).
- **`confirm()` se puede apagar desde Chrome** ("impedir que esta página cree más
  diálogos") y desde entonces devuelve *no* sin mostrar nada. Cualquier botón detrás de
  un `confirm()` queda muerto en silencio. Se usa la capa propia del panel (T11.48).
- **Un `<dialog>` dentro de una pantalla que cambia el `src` de un iframe**: cada cambio
  de `src` agrega una entrada al historial de la ventana, y el parche del botón Atrás
  deja de coincidir con el historial real. El diálogo no cierra. En esas pantallas se
  usa un panel propio, no `<dialog>` (T11.49).
- **Subir a Cloudinary sin `asset_folder`**: la foto cae en la raíz, junto a los
  comprobantes de Dinero, y ningún listado por carpeta la encuentra. La cuenta está en
  modo Dynamic folders: la carpeta va en `asset_folder`, no en `folder` (T11.50).
- **Un comodín entre comillas en la búsqueda de Cloudinary**: `asset_folder:"cabanas/*"`
  busca una carpeta llamada literalmente así, asterisco incluido. **Sin comillas**
  funciona como prefijo. Con las comillas, las subcarpetas son invisibles (T11.52).
- **Cloudinary distingue mayúsculas en los nombres de carpeta**: buscar `espacios`
  cuando la carpeta es `Espacios` devuelve cero, con HTTP 200 y sin error (T11.52).
- **En modo Dynamic folders la carpeta NO viaja en la URL de la foto**: es un dato
  aparte del identificador. Desde el navegador es imposible saber dónde está guardada
  una foto; hace falta la Admin API. Cualquier pantalla que lo afirme sin consultarla
  está inventando (T11.51).
- **Un deploy de Netlify reemplaza el proyecto ENTERO**: no se suman funciones. Un
  paquete con menos funciones de las que hay apaga las que falten, sin preguntar. El
  proyecto tiene cinco y solo dos están en GitHub (T11.52).
- **Un ícono de Material Symbols en un sitio que carga Material Icons**: no da error,
  dibuja cualquier cosa. `mode_fan`, `table_restaurant` y `kayaking` son de Symbols
  (T11.53).
...
> `index.html` [RAÍZ] · `opiniones.html` [RAÍZ, NUEVO] · `sitemap.xml` ·
> `interno/opiniones.html` [NUEVO] · `interno/cargar-opiniones.html` [NUEVO,
> DE UNA SOLA VEZ] · `interno/textos-sitio.js` · `interno/nucleo.js` ·
> `interno/sw.js` → v96):**
>
> ## El pedido
>
> Que alguien que nos encuentra por primera vez —sin conocernos, sin
> recomendación de por medio— pueda **leer lo que dijeron los que ya vinieron**.
> Hasta ahora eso solo existía dentro de Airbnb.
>
> ## Lo primero fue un dato duro, no una decisión de diseño
>
> **Airbnb no tiene API pública.** Solo la da a socios aprobados. Todos los
> complementos que se venden por ahí —Elfsight, Tagembed, Repuso, Revyoos— **raspan
> la página del anuncio**: se rompen solos el día que Airbnb toca su HTML, son pagos,
> y meten JavaScript de un tercero en el sitio público. Se descartaron.
>
> **Las opiniones se copian a mano y se quedan quietas.** Es trabajo, pero es trabajo
> que no se rompe.
>
> ## T11.55 · La estructura, y una decisión corregida sobre la marcha
>
> **Dónde viven: `config/sitio`.** No hay colección nueva, **no hay regla de
> Firestore nueva** y no hay una segunda lectura por visita — el sitio ya bajaba ese
> documento. Es coherente con lo que la propia documentación dice de `config/sitio`:
> *"el único lugar del contenido público"*.
>
> ```
> opiniones: [ { autor, fecha:'2026-03', idioma, estrellas, texto, orden } ]
> airbnb:    { c1: { url, nota, resenas }, c2: {…}, c3: {…} }
> ```
>
> **NO van en `textos-sitio.js`.** Ese archivo ya tiene 108 claves por idioma. Una
> opinión no es una etiqueta de interfaz: es un dato que crece. Si entrara ahí, cada
> opinión nueva sería editar un archivo de código en tres idiomas y subir el service
> worker.
>
> **NO se traducen.** Una opinión es testimonio ajeno: pasarla por un traductor la
> convierte en texto nuestro y deja de valer como prueba. Se muestra en el idioma en
> que la escribieron, con la etiqueta al lado. De paso ahorra dos tercios del trabajo
> de carga.
>
> **La dirección del anuncio SÍ es contenido público**, a diferencia del `.ics`. El
> criterio de la T11.44 para sacar las direcciones iCal de `cabanas` era de higiene:
> esa colección la baja cada visitante. La URL del anuncio *está hecha* para
> mostrarse. Vive en `config/sitio.airbnb` y no en `cabanas` por comodidad: así se
> carga desde la misma pantalla que las opiniones, sin tocar `cabanas.html`.
>
> **Decisión corregida a mitad de camino, y vale registrarla.** El diseño acordado
> repartía las opiniones **por cabaña**. Al mirar las reseñas de verdad, casi ninguna
> nombra un departamento: hablan de la casa, de la piscina, del jardín y de quienes
> reciben. Repartirlas habría inventado una división que los huéspedes no hicieron, y
> con doce reseñas quedaban tres por tarjeta. **Quedan en una sección propia**, justo
> antes del formulario de consulta —la prueba, y enseguida el pedido—; cada tarjeta
> conserva **su** nota y **su** enlace, que sí son de cada anuncio.
>
> ## El enlace a Airbnb: la tensión, dicha sin vueltas
>
> Mandar al visitante a Airbnb es mandarlo al lugar donde puede reservar con comisión.
> Por eso el enlace **dice "verificar", no "ver el anuncio"**, va último, chico y en
> gris, y lleva `rel="nofollow"` — no hay motivo para regalarle posición en Google al
> que nos cobra comisión. Es una prueba, no una segunda vidriera.
>
> ## T11.56 · Una página propia, y las claves donde corresponde
>
> **`opiniones.html` en la RAÍZ.** Mismo criterio que `la-casa.html`: la portada tiene
> que convencer a quien está mirando, esta tiene que **poder mandarse por WhatsApp** a
> quien pregunta si hay referencias. Una dirección propia se comparte, se guarda y se
> cita.
>
> ⚠ **HAY DOS `opiniones.html` Y NO ES UN ERROR.** El de la raíz es público (lee y
> muestra); el de `interno/` es el cargador (escribe). Es el mismo par que ya existe
> con `recuerdos.html`.
>
> Las claves nuevas —`opi_t`, `opi_p`, `opi_ver`, `opi_en`, `opi_mas`, `opi_menos`,
> `opi_volver`— entran a `textos-sitio.js` en los tres idiomas. En la T11.55 habían
> quedado en un `T_EXTRA` dentro del `index.html`, y mientras estuvieron ahí **el
> editor visual no las veía**: no se podían traducir desde la pantalla. La deuda quedó
> marcada con ⚠ en el código y en el registro del `sw.js`, y se cerró en la tanda
> siguiente. `sitemap.xml` suma `/opiniones.html` (§7.9).
>
> ## T11.57 · La carga, las caras y las fechas
>
> **`cargar-opiniones.html`, herramienta de una sola vez** (§7.7: fuera del menú,
> fuera del `SHELL`, se borra cuando terminó). Escribe **solo** `opiniones`, con
> `merge`: las notas y las direcciones que ya estén cargadas no se tocan. Veintiséis
> opiniones tipeadas a mano en un teléfono no las carga nadie; el contenido va en la
> herramienta y no en el código que queda.
>
> **NO se copian las fotos de perfil.** Son caras de personas reales alojadas en el
> servidor de Airbnb: traerlas es republicar la imagen de alguien sin haberle
> preguntado —y en Brasil eso cae bajo la LGPD—, y enlazarlas se rompe el día que
> Airbnb cambie una dirección. En su lugar va **la inicial en un círculo de color**,
> derivado del propio nombre por una suma sin estado: el mismo nombre da siempre el
> mismo color y no se guarda nada. Es lo que hace el propio Airbnb con quien no tiene
> foto.
>
> **Las fechas que no se saben quedan VACÍAS.** Donde Airbnb mostraba el mes exacto,
> está el mes exacto. Donde solo decía "hace 1 año", el campo va en blanco y el sitio
> simplemente no muestra el mes. Una fecha calculada a ojo es un dato inventado.
>
> ## El criterio de qué se publica — la línea, escrita
>
> **Elegir qué opiniones se muestran es normal; maquillar la NOTA no.** Se dejaron
> afuera las dos críticas (Jairo 3★, Elena 4★), una demasiado corta para decir algo, y
> una escrita entera en mayúsculas. Pero **las tres notas van completas**, el 4.6 del
> Loft incluido: publicar el 5.0 y el 4.93 y saltear el 4.6 sería engañar por omisión,
> y se destapa solo con el enlace de verificar que está al lado. Se dejaron adentro
> dos reseñas de 5★ que traen un reparo (cortina blackout, microondas): el reparo hace
> creíbles a las otras veinticuatro.
>
> ## Cómo se sabe qué texto copiar (va al manual, no al código)
>
> En la línea gris debajo de cada reseña de la aplicación de Airbnb:
> · **"Traducir al español"** es una oferta → lo que se ve **ya es el original**.
> · **"Traducido del portugués. Mostrar original"** → lo que se ve es la traducción;
>   hay que tocar *Mostrar original*.
>
> Se documenta porque el primer intento de carga fue con las traducciones automáticas
> a la vista, creyendo que eran el texto de la persona.
>
> ## Los tres anuncios, al 30-ago-2026
>
> | Anuncio (nombre en Airbnb) | Nuestro nombre | Nota | Reseñas |
> |---|---|---|---|
> | Cabaña vista al bosque | Cabaña con vista al bosque | 5.0 | 12 |
> | Departamento ideal para familias | Departamento para familias | 4.93 | 14 |
> | Loft en Canasvieiras cerquita de la playa | Loft con terraza | 4.6 | 5 |
>
> Suman 31, que es lo que muestra el perfil de Florencia: **el perfil de la anfitriona
> agrega los tres anuncios**. Los nombres de Airbnb no coinciden con los del sistema;
> al cargar hay que asignar cada uno a su cabaña a mano.
>
> ## Reparos operativos encontrados en las reseñas (no son del sitio)
>
> 1. **Cortina blackout** en las ventanas — el sol despierta temprano (Djonatan).
> 2. **Microondas** en la cabaña (Willy).
> 3. **Olor en las piezas y heladera sucia** — control de limpieza (Elena, 4★).
> 4. **Ruido del piso de madera** entre planta alta y baja (Elena). No tiene arreglo
>    barato: conviene decirlo en `la-casa.html` antes de que lleguen.
>
> ## Error propio de esta serie, registrado dos veces

Las tres tandas se numeraron primero **11.46, 11.47 y 11.48**, números que el
repositorio ya tenía usados. Se renumeraron a **11.51, 11.52 y 11.53** — y esos
**también estaban usados**: en paralelo, otra sesión había hecho `fotos.html` (11.51),
`cloudinary-listar` (11.52), los bloques ocultos (11.53) y el arreglo del guardado
(11.54). Quedan como **11.55, 11.56 y 11.57**, que es donde corresponden: sus service
workers son la v95 y la v96, o sea posteriores a la v94 de aquella serie.

**La causa, las dos veces:** se trabajó contra la copia del briefing que está en el
conocimiento del proyecto, que termina en la **v5.68 / T11.23**. La segunda vez, además,
el número se dedujo del último commit del repositorio sin mirar si había trabajo en
curso sin subir.

**Regla que sale de acá:** el número de tanda no se deduce ni del briefing del
conocimiento ni del último commit. Sale del **`sw.js` que se está por entregar**: su
`VERSION` es el único contador que avanza una vez por tanda y que nadie puede saltear,
porque sin tocarlo la entrega no llega a los teléfonos.

## Acción
>
> 1. Subir a la **raíz**: `index.html`, `opiniones.html`, `sitemap.xml`.
> 2. Subir a **`interno/`**: `opiniones.html`, `cargar-opiniones.html`,
>    `textos-sitio.js`, `nucleo.js`, `sw.js` (v96).
> 3. Cerrar la app por completo y reabrirla.
> 4. Abrir `interno/cargar-opiniones.html` a mano y tocar el botón.
> 5. **Más → Sitio → Opiniones**: cargar las tres notas y las tres direcciones.
> 6. **Borrar `interno/cargar-opiniones.html`** del repositorio (§7.7).
>
> **Sin cambios en `firestore.rules`.** Todo va a `config/sitio`, que ya tiene lectura
> pública y escritura con permiso `contenido`.
>
> ## Pendiente
>
> Faltan las direcciones de dos de los tres anuncios. La de la cabaña, limpia y sin
> los parámetros de compartir: `https://www.airbnb.com/rooms/1337276103911358917`.

---

# v5.69 — El acuerdo de convivencia, el traductor y el orden de las fotos (T11.45 a T11.54)

> **Registro v5.69 (Tandas 11.45 a 11.54 — 27 y 28 de agosto de 2026):**
>
> Nueve tandas seguidas sobre el sitio público y sus herramientas. Empezó por meter
> en el sitio lo que hasta ahora solo vivía en la ficha de Airbnb, y terminó
> destapando cuatro fallos que estaban ahí desde antes y que nadie veía.
>
> ---
>
> **T11.45 · Nace la página del acuerdo.** El contenido de Airbnb —servicios, reglas,
> horarios, condiciones— entra al sitio propio. No como sección de la portada sino
> como página propia, **`la-casa.html`** en la raíz: la portada tiene que convencer a
> alguien que todavía no reservó; esta tiene que poder **mandarse por WhatsApp antes
> del check-in y leerse entera**. Son dos trabajos distintos y juntos se estropean.
> La portada suma un resumen de cuatro datos y el enlace. `textos-sitio.js` pasa de
> 33 a 108 claves por idioma. `sw.js` → v85.
>
> **Cada regla lleva su porqué debajo.** Es deliberado y no se saca: una regla sin
> motivo se discute, una regla con motivo se entiende. Es la diferencia entre un
> reglamento y un acuerdo, y acuerdo es la palabra que usó Mauro.
>
> **Diagnóstico corregido en el camino.** Se dijo que había que traer de CasaYourte el
> mecanismo de piezas. **Ya estaba**: el index marca cada texto con `data-t`, el editor
> deriva el `data-edit` solo, y `config/sitio.textos` acepta cualquier cantidad de
> claves. Las "nueve claves" son del formulario viejo de `cabanas.html`, no del editor
> visual. La tanda resultó mucho más chica de lo planteado.
>
> **Lo que varía por apartamento salió de la página general.** Aire acondicionado,
> capacidad y acceso por escalera son distintos en cada uno: escribirlos como si
> fueran de todos dejaba mal descritos a dos de tres. Van en `cabanas/{id}.amenities`,
> que el index ya pinta. La página general manda a mirar la ficha.
>
> ---
>
> **T11.46 · Revisar y traducir, y un defecto de la v85 cazado antes de que se viera.**
> Nace **`traducir.html`**: saca TODO el español con su contexto y el pedido de
> auditoría adentro, y mete de vuelta las correcciones pieza por pieza. El contexto
> **no se escribe a mano**: se lee de `index.html` y `la-casa.html` abiertos en marcos
> ocultos con `?edit=1`, en orden de lectura. Estado **por idioma destino**: una pieza
> puede estar al día en portugués y vieja en inglés.
>
> **⚠ EL DEFECTO: las claves con punto no se pueden guardar.** El editor usa `updateDoc`
> con el camino `textos.es.LACLAVE`, y Firestore lee **cada punto como un nivel de
> anidamiento**. `casa.r.ruido.t` se habría guardado en `textos→es→casa→r→ruido→t`
> mientras el sitio lee la clave literal. El texto se guarda, dice "✓ Sitio
> actualizado" y **no se muestra nunca**. Con las claves viejas (`titulo`, `c1`) no
> podía pasar porque ninguna tenía puntos. Todas las claves nuevas pasan a **guión
> bajo** (`casa_r_ruido_t`) y `editar.html` se niega a guardar una clave con punto en
> vez de fallar callado. `sw.js` → v86.
>
> También: el silencio pasa a **0:00–8:00**, y "Editar el sitio" entra a la barra
> (antes solo se llegaba desde Cabañas).
>
> ---
>
> **T11.47 · El enlace a la página nueva estaba roto.** Al renombrar las claves de punto
> a guión bajo, el reemplazo por patrón **también alcanzó la cadena `casa.html` dentro
> de `la-casa.html`**: el guión cuenta como límite de palabra. Los dos enlaces de la
> portada, el `canonical` y el `og:url` quedaron apuntando a `/la-casa_html` — 404,
> con Google indexando una dirección inexistente y la previsualización de WhatsApp
> caída. `sw.js` → v87.
>
> **Lección:** un reemplazo masivo por patrón se verifica contra **las rutas y las URLs**
> del archivo, no solo contra las claves que se querían tocar.
>
> ---
>
> **T11.48 · Dos botones muertos en el editor.** "Descartar" y "Volver" usaban
> `confirm()`, que Chrome deja apagar con *"impedir que esta página cree más
> diálogos"*. Apagado, `confirm()` devuelve *no* sin mostrar nada: Descartar no
> descartaba y Volver no dejaba volver, los dos en silencio. Pasan a la capa propia
> del panel, que además entra en la pila del botón Atrás. `sw.js` → v88.
>
> ---
>
> **T11.49 · La hoja de fotos no se cerraba y trababa el guardado.** Era un `<dialog>`
> nativo y su cierre pasaba por el parche de `nucleo.js`, que engancha el Atrás
> moviendo el historial. Pero **este editor cambia el `src` de un iframe** —al cambiar
> de página y al recargar el sitio— y cada cambio de `src` **agrega una entrada al
> historial de la ventana**: la pila de capas y el historial real dejan de coincidir y
> el `history.back()` del cierre retrocede una navegación del iframe. La hoja quedaba
> abierta tapando la barra de abajo, con el botón Guardar adentro: **se podía editar y
> no se podía guardar**. Pasa a panel propio. `sw.js` → v89.
>
> ---
>
> **T11.50 · Las fotos se subían a la raíz.** `CV2.subirImagen` **nunca mandaba
> carpeta**, así que TODAS las fotos del sistema —las del sitio, las de los
> alojamientos, los avatares— caían en la raíz de Cloudinary, mezcladas con los
> comprobantes de Dinero. Por eso el listado del editor no encontraba ninguna: buscaba
> en carpetas vacías. Ahora `subirImagen(file, carpeta)` manda **`asset_folder`** y el
> editor deduce el destino de lo que se está editando. Mapa único en
> `CV2.CLOUDINARY.carpetas`. `sw.js` → v90.
>
> ---
>
> **T11.51 · Revisar las fotos.** Nace **`fotos.html`**: junta todas las fotos que
> muestra el sitio y las mira una por una. Encuentra las servidas por otro sitio (y
> las trae a Cloudinary de a una, con confirmación), las que no cargan, las que se
> piden sin la transformación de entrega y las repetidas.
>
> **Lo que NO puede saber, y se dice en vez de fingir:** en qué carpeta está guardada
> cada foto. En modo Dynamic folders la carpeta es un dato aparte del identificador y
> **no viaja en la URL**. Si `cloudinary-listar` responde, cruza y marca las mal
> ubicadas; si no, muestra el identificador y la carpeta que corresponde, para
> acomodarlas a mano. `sw.js` → v91.
>
> ---
>
> **T11.52 · `cloudinary-listar`, y dos diagnósticos equivocados.**
>
> **Primer error:** se dio por hecho que la función no existía, mirando la carpeta
> `netlify/functions` de GitHub. Ahí solo están dos de las cinco: **las otras tres
> viven únicamente dentro de Netlify**, porque el proyecto no está enganchado a Git.
>
> **Segundo error:** se dio por hecho que buscaba por `folder` en vez de
> `asset_folder`. **No era así** — ya usaba `asset_folder`, con un comentario
> explicando por qué. La función existente está mejor escrita que el reemplazo
> propuesto, así que **se corrigió en lugar de reemplazarla**.
>
> **Los dos fallos reales, ninguno conceptual:**
> 1. `PERMITIDAS` decía **`'espacios'` en minúscula** y la carpeta se llama
>    `Espacios`. Cloudinary distingue y devuelve cero sin error. Las áreas comunes
>    eran invisibles. Ahora usa el nombre real y `permitida()` ignora mayúsculas.
> 2. **El comodín iba entre comillas**: `asset_folder:"cabanas/*"` busca una carpeta
>    llamada literalmente así, asterisco incluido. Sin comillas funciona como prefijo.
>    Como `cabanas` está vacía y todo cuelga de `cabana1..3`, no aparecía ninguna foto
>    de alojamiento.
>
> Se agrega `publicId` junto a `public_id` en la respuesta, que es como lo lee
> `fotos.html`.
>
> **⚠ UN DEPLOY DE NETLIFY REEMPLAZA EL PROYECTO ENTERO.** El zip tiene que llevar
> SIEMPRE las cinco funciones. Un paquete con menos apaga las que falten, sin
> preguntar. Se estuvo a punto de soltar uno con tres.
>
> ---
>
> **T11.53 · Poder sacar algo del sitio, y el ícono del ventilador.**
>
> `mode_fan` **no existe en Material Icons**: es de la fuente Symbols, que el sitio no
> carga. Cambia a `air`. De paso `table_restaurant` → `dining` y `kayaking` →
> `rowing`, que estaban en el mismo riesgo.
>
> **Lo de fondo:** vaciar un texto no lo borra, lo devuelve al diccionario — y eso es
> correcto, porque quien borra sin querer no debería dejar un hueco para siempre. Pero
> entonces no había forma de sacar del sitio una regla que ya no corresponde. Nace
> **`config/sitio.ocultos`**: `textos` guarda lo que dice, `ocultos` guarda lo que no
> se muestra. Dos intenciones distintas, dos campos distintos.
>
> En el sitio se esconde el texto **y el contenedor cuyos textos estén TODOS
> escondidos**, para que no quede un renglón vacío con el ícono suelto. Editando no
> desaparece: se ve apagado y tachado, con un botón de ojo para traerlo de vuelta —
> sacar algo sin poder deshacerlo no es una función, es una trampa. `sw.js` → v93.
>
> **⚠ Lo que sigue sin poder hacerse:** AGREGAR un párrafo o una regla nueva pide tocar
> el HTML. Es la contrapartida de que cada renglón sea su propia clave. Cambiarlo
> pediría un modelo de bloques dinámicos, que es otra cosa y bastante más grande.
>
> ---
>
> **T11.54 · `invalid-argument` al guardar después de usar el ojo.** La lista de ocultos
> se leía de dentro del iframe y se guardaba tal cual. Ese array lleva el
> `Array.prototype` **del iframe**: para JavaScript es un array igual —`Array.isArray`
> dice que sí— pero Firestore comprueba el tipo contra **su** `Array`, no lo reconoce y
> **rechaza la escritura entera**, sin decir cuál de los campos fue. Se ocultaba algo, se
> tocaba Guardar y no se guardaba nada, ni siquiera los textos que no tenían que ver.
> Se copia con `Array.from()` al cruzar la frontera del marco, más un último filtro en
> `guardar()`. Los arrays de fotos nunca fallaron porque `arrayActual()` los construye en
> el editor desde cero: `ocultos` era el único dato que llegaba armado desde adentro.
> `sw.js` → v94.
>
> ---
>
> **Archivos de la serie.** Sitio: `index.html`, `la-casa.html`, `sitemap.xml`.
> Panel: `interno/textos-sitio.js`, `interno/editar.html`, `interno/traducir.html`,
> `interno/fotos.html`, `interno/nucleo.js`, `interno/sw.js` (v85 → v94).
> Netlify: `netlify/functions/cloudinary-listar.js`.

---

# v5.68 — Avisos, el acuerdo, y una noche de fósiles (T11.18 a T11.44)
...
> · **Síntoma**: la app quedaba trancada al loguearse la cuenta admin.
>   Solo con esa cuenta.
...
>   `[admin]` → **CasaVerde** (dueño de 35 sesiones) ·
>   `[mauro]` → **Mauro** (`[mauro-v1]` en el 2.0) ·
...
> email activo). Usuarios de Auth: [admin] (admin),
> florenciadetp, estebanzapata96, masotromauro
> — los tres últimos rol 'colaborador' en /usuarios/.
...
> **Novedad de la v1.7 (tandas 11.55 a 11.57).** El sitio suma **`opiniones.html`**: las
> reseñas de Airbnb, copiadas a mano y quietas. Airbnb no tiene API pública y los widgets
> que se venden raspan la página del anuncio. Las reseñas y las notas viven en
> `config/sitio`; **no se traducen**, porque son testimonio ajeno. Ver §4 y §14.

> **Novedad de la v1.6 (tandas 11.45 a 11.54).** El sitio deja de ser una sola página:
> nace **`la-casa.html`**, el acuerdo de convivencia. Los textos pasan de 33 a 108
> claves por idioma, aparece la lista `ocultos`, y el panel suma dos pantallas para
> mantener todo esto: **`traducir.html`** y **`fotos.html`**. Ver §2, §4 y §13.
...
| Ruta | Qué hace |
|---|---|
| `/index.html` | La portada: hero, espacios, áreas comunes, **resumen de la casa**, calendario y pie |
| `/la-casa.html` | **El acuerdo de convivencia** (T11.45). Qué hay, qué no hay, horarios, las diez reglas con su motivo, cámaras, estadías largas. Página propia y no una sección de la portada: la portada convence a quien no reservó, esta se manda por WhatsApp antes del check-in y se lee entera |
| `/opiniones.html` | **Las reseñas de Airbnb** (T11.56). Dirección propia para mandar por WhatsApp a quien pregunta si hay referencias. **Ojo: hay otro `opiniones.html` en `/interno/`**, que es el cargador. Mismo par que `recuerdos.html` |
...
| `/sitemap.xml` | Las **cuatro** URLs públicas. Al agregar o renombrar una página se toca este archivo Y el mapa de `404.html` |
...
### Las pantallas del panel que mantienen el sitio

| Ruta | Qué hace |
|---|---|
| `/interno/editar.html` | Edición visual **encima del sitio real**, no sobre una copia. Selector de página (Portada · La casa), los tres idiomas, fotos y el botón de ocultar |
| `/interno/traducir.html` | Saca todo el español con su contexto y el pedido de auditoría adentro; trae de vuelta las correcciones pieza por pieza (T11.46) |
| `/interno/fotos.html` | Revisa las fotos del sitio: las que están fuera de Cloudinary, las que no cargan, las que se sirven pesadas, las repetidas (T11.51) |
| `/interno/opiniones.html` | Carga las reseñas, las notas y las direcciones de los anuncios (T11.55) |

Las tres se alcanzan desde la barra, en el grupo **sitio**, con permiso `contenido`.
Hasta la T11.46 el editor solo se alcanzaba desde Cabañas: una pantalla a la que no
lleva ningún camino visible es una pantalla que no existe para quien no la escribió.
...
En código: `TEXTOS[lang][k] || T[lang][k] || T.es[k] || k`.

**El último respaldo pasó de `pt` a `es` (T11.45).** El español es el ORIGINAL del
sitio: es el idioma en que se escribe y desde el que se traduce. El respaldo tiene que
ser el original, no uno de los destinos.
...
### 4.2 · Las claves: nueve en el formulario, 108 en el editor visual

**Hay dos caminos distintos para editar un texto y no hay que confundirlos.**

| | Formulario de `cabanas.html` | Editor visual (`editar.html`) |
|---|---|---|
| Qué edita | **Nueve claves fijas** | **Cualquier cosa con `data-t`** — hoy 108 por idioma |
| Cómo se ve | Campos en una lista | Escribiendo encima del sitio real |
| Páginas | Solo la portada | Portada y La casa |

Sumar una clave a `textos-sitio.js` **NO la agrega al formulario**: la agrega al editor
visual, que es donde se escribe casi todo desde la T11.45.

> **Se creyó lo contrario durante una tanda entera.** Se planteó traer de CasaYourte el
> mecanismo de "piezas" para poder sumar textos sin tocar código. Ya estaba: el index
> marca cada texto con `data-t`, el editor deriva el `data-edit` solo, y
> `config/sitio.textos` acepta cualquier cantidad de claves. Las "nueve" son del
> formulario viejo. **Antes de planear un mecanismo, leer si ya existe.**

**Las claves de `la-casa.html` van con guión bajo** (`casa_r_ruido_t`), nunca con punto.
El motivo está en CONVENCIONES §4 y §8: `updateDoc` lee cada punto como un nivel de
anidamiento y el texto se guarda donde el sitio no lo busca, **sin dar error**.

**Cada renglón de lista es su propia clave** (`casa_lp_1`, `casa_lp_2`…) y no un array.
Así el editor visual y el traductor los toman con el mecanismo que ya existe, sin código
nuevo y sin codificar nada para Firestore. **El precio:** la *cantidad* de renglones vive
en el HTML, así que agregar uno pide tocar la página. Para un reglamento que cambia una
vez por año, es el precio correcto.

### 4.2b · Sacar algo del sitio: `ocultos` (T11.53)

Vaciar un texto **no lo borra**: lo devuelve al diccionario. Eso es lo correcto —quien
borra sin querer no debería dejar un hueco permanente— pero dejaba sin salida el caso
real, que es sacar una regla que ya no corresponde.

`config/sitio.ocultos` es una lista de claves que el sitio no muestra. **`textos` guarda
lo que dice; `ocultos`, lo que no se muestra.** Se esconde el texto Y el contenedor cuyos
textos estén todos escondidos, para que no quede un renglón vacío con el ícono suelto.

En el editor se toca un texto y aparece el **botón de ojo** en la barra de abajo. Lo
oculto **no desaparece mientras se edita**: se ve apagado y tachado, con el ojo para
traerlo de vuelta. Sacar algo sin poder deshacerlo no es una función, es una trampa.

**⚠ Lo que sigue sin poder hacerse:** *agregar* un párrafo o una regla nueva. Es la
contrapartida de que cada renglón sea su propia clave.

### 4.2c · Las nueve claves del formulario
...
> **Auditado (agosto 2026, T11.53):** **108 claves por idioma y los tres definen
> exactamente el mismo conjunto** — que es la comprobación que importa, porque una clave
> presente en español y ausente en portugués solo se nota al cambiar de idioma. Ninguna
> lleva punto. Toda clave usada con `data-t` en `index.html` y `la-casa.html` existe en
> los tres idiomas.
>
...
5. **El `sitemap.xml` se actualiza a mano.** Sus **tres** URLs corresponden a archivos
   que existen (T11.45). **El mapa de `404.html` NO se actualizó** con `la-casa.html`.
...
13. **Datos del sitio pendientes de cargar en el panel** (T11.53): la `capacidad` de cada
    apartamento (7 · 6 · 4) y los `amenities` que la página general manda a mirar —
    `Escalera de acceso` y `Terraza privada` en la Cabaña y el Loft; `Planta baja, sin
    escaleras`, `Baño adaptado`, `Aire acondicionado` y `Mesa exterior propia` en el
    Departamento para familias. **Sin esto, `casa_varia_p` manda a una ficha que no lo
    dice**, y la portada puede mostrar una capacidad y el texto otra.
14. **La ficha de Airbnb quedó con dos cosas mal declaradas** (T11.45): *"Alojamiento de
    un solo piso · sin escaleras"* como característica general —solo vale para uno de
    tres— y *"Servicios básicos"* entre los no incluidos, que en el vocabulario de
    Airbnb significa que no hay toallas, papel ni jabón. **Sin confirmar si es cierto.**
15. **`traducir.html` escribe en `config/traduccion`**, un documento nuevo. Si el bloque
    general de reglas de `config` no lo cubre, al aplicar aparece *"El registro no se
    pudo guardar"* — **los textos igual quedan publicados**, porque el registro se
    escribe después y en su propio intento. Verificar `firestore.rules`.
16. **Tres de las cinco funciones de Netlify no están en GitHub** (T11.52):
    `airbnb-ical`, `ical-cabana` y `cloudinary-listar` existen **únicamente dentro de
...
18. **Claves huérfanas en `textos-sitio.js`**: `lIn`, `lOut` y `ocupado`, traducidas a
    los tres idiomas y sin uso. Cada exportación del traductor las arrastra.
19. **`interno/cargar-opiniones.html` hay que borrarlo del repositorio** una vez cargadas
    las opiniones (§7.7: herramienta de una sola vez, fuera del menú y fuera del `SHELL`).
20. **Faltan las direcciones de dos de los tres anuncios de Airbnb** (T11.55). La de la
    cabaña, limpia y sin los parámetros de compartir:
    `https://www.airbnb.com/rooms/1337276103911358917`.
21. **Reparos operativos que salieron de las reseñas** (T11.57) — no son del sitio, son de
    la casa: cortina blackout en las ventanas, un microondas en la cabaña, control de
    limpieza (olor en las piezas, heladera), y el **ruido del piso de madera** entre
    planta alta y baja. Este último no tiene arreglo barato: **conviene decirlo en
    `la-casa.html`** antes de que lleguen, no después.
22. **La documentación quedó pública al entrar al repositorio** (tanda 11.17). Los tres
...
## 13 · Cómo se mantiene el contenido (T11.45 a T11.53)

### 13.1 · El ciclo

1. **Escribir en español**, encima del sitio, con `interno/editar.html`.
2. **Exportar** desde `interno/traducir.html` y pegar el archivo en el chat.
3. **Revisar y traducir** en el chat: el pedido de auditoría viaja dentro del archivo.
4. **Traer de vuelta** el JSON, revisar pieza por pieza con casillas, aplicar.
5. **Revisar las fotos** en `interno/fotos.html` cuando se hayan tocado imágenes.

**El español es el original.** Se escribe en español y se traduce desde ahí; el respaldo
del sitio también cae en español.

### 13.2 · El traductor

**El contexto no se escribe a mano.** `traducir.html` abre `index.html` y `la-casa.html`
en marcos ocultos con `?edit=1` y recorre los `data-t` en orden de lectura, anotando de
cada texto su página, su sección, su bloque y su papel. Si el sitio cambia, el traductor
lo refleja sin que nadie toque una tabla.

**El pedido de auditoría vive en el código, no en el chat.** Una instrucción que se
redacta de nuevo cada vez da un resultado distinto cada vez, y entonces no se puede
comparar una revisión con la anterior. Pide cuatro cosas en este orden: ortografía y
gramática del español rioplatense (voseo), coherencia entre las dos páginas, estructura
y presentación, y recién después la traducción.

**El mapa del sitio viaja entero aunque el archivo salga por partes.** La coherencia y el
orden no se pueden juzgar mirando piezas sueltas.

**El estado es por idioma destino.** Una pieza puede estar al día en portugués y vieja en
inglés. Con un solo estado, corregir el español y traducir solo el portugués dejaba el
inglés roto sin que nada lo dijera.

**"Dar por al día" se usa UNA vez, al principio** — y **después** de aplicar las
correcciones pendientes. Al revés, las traducciones viejas quedan registradas como
buenas y el sistema deja de avisar.

**Auditoría mecánica local**, sin chat: espacios dobles, `¿`/`¡` faltantes, espacio antes
de puntuación, traducción idéntica al español, y **texto en portugués metido en el campo
español** — el sitio arrancó con todo por defecto en portugués, así que ese caso es real.

### 13.3 · Las fotos

**Cada foto va a su carpeta según qué se esté editando** (T11.50). El mapa único está en
`CV2.CLOUDINARY.carpetas`:

| Clave | Carpeta real en Cloudinary |
|---|---|
| `c1` · `c2` · `c3` | `cabanas/cabana1` · `cabana2` · `cabana3` |
| `espacios` | `Espacios` — **con E mayúscula, así está allá** |
| `sitio` | `sitio` (por defecto) |
| `recuerdos` · `comprobantes` | `recuerdos` · `gastos` |

**Los nombres se escriben con las mayúsculas reales.** Cloudinary distingue: buscar
`espacios` cuando la carpeta es `Espacios` devuelve cero, con HTTP 200 y sin error.

**Mover una foto en Cloudinary no cambia su dirección de entrega.** Por eso ordenar las
carpetas allá nunca rompe el sitio.

`cloudinary-listar` acepta **`?modo=carpetas`**: devuelve todas las carpetas con su
conteo y cuáles el editor puede ver. Es la primera consulta ante cualquier duda.

### 13.4 · El acuerdo de convivencia

**Cada regla lleva su motivo debajo, y no se saca.** Una regla sin porqué se discute; con
porqué se entiende. Es la diferencia entre un reglamento y un acuerdo.

**Lo que varía por apartamento no va en la página general.** Aire acondicionado,
capacidad y acceso por escalera están en `cabanas/{id}`; `casa_varia_p` manda a mirar la
ficha. Escribirlos como si fueran de todos deja mal descritos a dos de tres.

**Las cámaras se dicen en positivo**: decir dónde NO hay tranquiliza más que decir dónde sí.

**El tono es propio, no el de Airbnb.** Airbnb escribe *"Prohibido fumar"*; acá se dice
por qué el olor queda en los colchones y lo hereda el próximo huésped. Y nada de la
traducción mexicana de Airbnb: **piscina** y **ducha**, no "alberca" ni "regadera".

---

## 14 · Las opiniones (T11.55 a T11.57)

**Airbnb no tiene API pública.** Solo la da a socios aprobados. Los complementos que se
venden —Elfsight, Tagembed, Repuso, Revyoos— **raspan la página del anuncio**: se rompen
el día que Airbnb toca su HTML, son pagos, y meten JavaScript de un tercero en el sitio
público. Se descartaron. **Las opiniones se copian a mano y se quedan quietas.** Es
trabajo, pero es trabajo que no se rompe.

### 14.1 · Las decisiones que no se cambian sin volver a discutirlas

**No se traducen.** Una opinión es testimonio ajeno: pasarla por un traductor la convierte
en texto nuestro y deja de valer como prueba. Se muestra en el idioma original con la
etiqueta al lado.

**No se copian las fotos de perfil.** Son caras de personas reales alojadas en el servidor
de Airbnb: traerlas es republicar la imagen de alguien sin preguntarle —en Brasil eso cae
bajo la LGPD— y enlazarlas se rompe cuando Airbnb cambie una dirección. En su lugar va la
inicial en un círculo de color, derivado del nombre por una suma sin estado: el mismo
nombre da siempre el mismo color y no se guarda nada. Es lo que hace el propio Airbnb con
quien no tiene foto.

**Las fechas que no se saben quedan vacías.** Donde Airbnb mostraba el mes exacto, está el
mes exacto; donde decía "hace 1 año", el campo va en blanco y el sitio no muestra nada.
Una fecha calculada a ojo es un dato inventado.

**Elegir qué opiniones se muestran es normal; maquillar la NOTA no.** Se dejaron afuera
dos críticas, una demasiado corta y una escrita en mayúsculas. Pero **las tres notas van
completas**, el 4.6 del Loft incluido: publicar el 5.0 y el 4.93 y saltear el 4.6 sería
engañar por omisión, y se destapa solo con el enlace de verificar que está al lado. Se
dejaron adentro dos reseñas de 5★ que traen un reparo: el reparo hace creíbles a las
otras veinticuatro.

**El enlace a Airbnb es una prueba, no una segunda vidriera.** Mandar al visitante a
Airbnb es mandarlo donde puede reservar con comisión. Por eso dice **"verificar"** y no
"ver el anuncio", va último, chico y en gris, y lleva `rel="nofollow"` — no hay motivo
para regalarle posición en Google a quien nos cobra comisión.

**Van en una sección propia y no repartidas por cabaña.** El diseño acordado las repartía;
al mirar las reseñas de verdad, casi ninguna nombra un departamento: hablan de la casa, la
piscina, el jardín y de quienes reciben. Repartirlas habría inventado una división que los
huéspedes no hicieron. Cada tarjeta sí conserva **su** nota y **su** enlace, que son de
cada anuncio.

### 14.2 · Cómo se copia el texto correcto

En la línea gris debajo de cada reseña, en la aplicación de Airbnb:

- **"Traducir al español"** es una oferta → lo que se ve **ya es el original**.
- **"Traducido del portugués. Mostrar original"** → lo que se ve es la traducción; hay
  que tocar *Mostrar original*.

Se documenta porque el primer intento de carga fue con las traducciones automáticas a la
vista, creyendo que eran el texto de la persona.

### 14.3 · Los tres anuncios, al 30-ago-2026
...
Suman 31, que es lo que muestra el perfil de Florencia: **el perfil de la anfitriona
agrega los tres anuncios**. Los nombres de Airbnb no coinciden con los del sistema; al
cargar hay que asignar cada uno a su cabaña a mano.
